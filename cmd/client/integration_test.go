...
func TestManagerClientEnrollmentToolCallAndCredentialReconnect(t *testing.T) {
	...
	go func() {
		firstDone <- runSession(firstCtx, managerURL, firstReg, service, func(credential string) error {
			issued <- credential
			return nil
		})
	}()
	...
	go func() {
		secondDone <- runSession(secondCtx, managerURL, secondReg, service, func(newCredential string) error {
